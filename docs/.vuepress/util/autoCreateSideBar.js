...
module.exports = {
  ...
  getFileCatalogue(path = '', white_path = []) {
    // 1. 过滤掉白名单的文件
    const catagolue_list = fs
      .readdirSync('./docs' + path)
      .filter((file) => !white_path.includes(file))
    ...
    file_catalogue[path + '/'] = [
      {
        ...
        children: catagolue_list
          ...
          .map((file) => {
            return file === 'README.md' || white_path.includes(file)
              ? ''
    ...
    catagolue_list
      .filter((v) => !v.includes('.md'))
      .forEach((new_path) =>
        this.getFileCatalogue(path + '/' + new_path, white_path)
      )
  },
